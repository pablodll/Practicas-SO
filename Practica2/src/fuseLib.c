...
static int my_unlink(const char *path)
{
   ...
   // Buscamos el indice del fichero
   if((fileIndex = findFileByName(&myFileSystem, (char *)path + 1)) == -1){
      return -ENOENT;
   }
   
   iNode = myFileSystem.directory.files[fileIndex].nodeIdx;

   // Cambiamos el tamaño del fichero apuntado por iNode a 0
   ...
   // Modificamos el numero de nodos y de bloques libres
   ...
   updateBitmap(&myFileSystem);
   updateDirectory(&myFileSystem);
   updateNode(&myFileSystem, fileIndex, myFileSystem.nodes[fileIndex]);
   updateSuperBlock(&myFileSystem);
